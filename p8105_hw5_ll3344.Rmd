...
The second plot shows that the sample average of mu across tests for which the null is rejected is higher than the true mean for smaller true means but approaches the true mean as the true mean gets larger. This makes sense as almost all samples of true mean 6, for example, will be rejected. However, as we saw in the previous plot, for true mean of 1, only 20% of samples will be rejected and these samples will have a higher mean than 1. As the current plot shows, the mean of rejected samples falls around 2.5 with true mean 0. 
